...
TEST(ClassinputTest, IsValidTest) {
    ...
    EXPECT_EQ(IsValid(data1), false);
    EXPECT_EQ(IsValid(data2), true);
    EXPECT_EQ(IsValid(data3), false);
}

TEST(UsefulFunctionsTest, IsLetterOr1Test) {
    EXPECT_EQ(IsLetterOr1('a'), true);
    EXPECT_EQ(IsLetterOr1('b'), true);
    EXPECT_EQ(IsLetterOr1('c'), true);
    for (int i = 1; 'c' + i <= 'z'; ++i) {
...
TEST(RegTest, PushLetterOr1Test) {
    ...
    PushLetterOr1(result_reg_stack, 'a', data);
    ...
    PushLetterOr1(result_reg_stack, 'b', data);
...
TEST(AlgoTest, TaskTest) {
    ...
    EXPECT_EQ(Result(data1), true);
    data1.amount = 1;
    EXPECT_EQ(Result(data1), false);
    data1.amount = 3;
    EXPECT_EQ(Result(data1), false);
    datainput data2("ab.a+c.a.*a.b.a+", 'a', 1);
    EXPECT_EQ(Result(data2), true);
    data2.amount = 2;
    EXPECT_EQ(Result(data2), false);
    data2.amount = 115;
    EXPECT_EQ(Result(data2), true);
    datainput data3("aab.*c+a.*ab.+.c.", 'a', 2);
    EXPECT_EQ(Result(data3), true);
    data3.amount = 161;
    EXPECT_EQ(Result(data3), true);
    data3.amount = 0;
    EXPECT_EQ(Result(data3), false);
}
